...
bridge:
  ...
  group = SET_2; //<-- start group (customize)
...
void action()
{
  unsigned long commandeGauche = 1;  
  unsigned long commandeDroite = 2;  
  unsigned long commandeAvant = 3;  
  unsigned long commandeArriere = 4;
  switch (group)
  {
  case SET_1:
    switch (idx)
  ...
  case SET_2:
    ...
    {
    case S2_LEFT:      
      radio.write( &commandeGauche, sizeof(unsigned long) );
      radio.write( &commandeGauche, sizeof(unsigned long) );
      // group = GROUP_X\SET_X; <-- or jump to another group or wordset for composite commands
      break;
    case S2_RIGHT:  
      radio.write( &commandeDroite, sizeof(unsigned long) );
      radio.write( &commandeDroite, sizeof(unsigned long) );   
      // group = GROUP_X\SET_X; <-- or jump to another group or wordset for composite commands
      break;
    case S2_UP:
    ...
    case S2_DOWN:
      ...
      break;
    case S2_FORWARD: 
      radio.write( &commandeAvant, sizeof(unsigned long) );     
      radio.write( &commandeAvant, sizeof(unsigned long) ); 
      // group = GROUP_X\SET_X; <-- or jump to another group or wordset for composite commands
      break;
    case S2_BACKWARD:
      radio.write( &commandeArriere, sizeof(unsigned long) );      
      radio.write( &commandeArriere, sizeof(unsigned long) ); 
      // group = GROUP_X\SET_X; <-- or jump to another group or wordset for composite commands
      break;
    }
